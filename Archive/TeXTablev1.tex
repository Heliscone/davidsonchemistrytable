...
%%FORMULAS%%
	%Periodic Trends
	\node at ($(Ra.south)-(-1,4.8)$) (TrendOne) [draw] {EN, IE, EA, \& $\mathbb{Z}_{eff}$ increase $\rightarrow\uparrow$};
	...
	\node at ($(Fr.south)-(0,1.8)$) (SuperSeven) [draw] [text width=1cm,align=left] {\textit{Super\textsuperscript{7}}\\ HI\\ HBr\\ HCl\\ HNO\textsubscript{3}\\ H\textsubscript{2}SO\textsubscript{4}\\ HClO\textsubscript{3}\\ HClO\textsubscript{4}};
	%Constants
...
$\dfrac{453.6\text{ g}}{1\text{ lb}}=\dfrac{2.54\text{ cm}}{1\text{ in}}=\dfrac{0.946\text{ L}}{1\text{ qt}}=\dfrac{6.022\times10^{23}\text{ units}}{\text{mol}}=\dfrac{22.4\text{ L}}{\text{mol}}$\\ \vspace{0.3cm}$\dfrac{1\text{ g}_{\text{water}}}{1\text{ mL}_{\text{water}}}=\dfrac{P_{\text{sea}}}{101325\text{ Pa}}=\dfrac{1\text{ atm}}{760\text{ mmHg}}=\dfrac{1\text{ atm}}{14.7\text{ psi}}=\dfrac{1\text{ kcal}}{4.184\text{ kJ}}$\\ \vspace{0.3cm} $^{\circ}\text{C}=\frac{5}{9}(^{\circ}\text{F}-32)$ and $^{\circ}\text{F}=\frac95 ^{\circ}\text{C}+32$ and $1\text{V}=\frac{\text{J}}{\text{C}}$\\ \vspace{0.3cm} $h=6.626\times10^{-34}\text{Js}\lor\text{kgm}^2\text{s}^{-1}$ and $c=3.00\times10^8\text{ ms}^{-1}$};
	%Gas
	...
	%Dimensions
	\node at ($(Gasses.north)+(0,0.4)$) (Dimen) [text width = 6cm] {[J]$=\text{kg m}^2\text{s}^{-2}$, $\text{[N]}=\text{kg m s}^{-1}$, \\ $\text{[Pa]}=\text{N m}^{-2},\text{ kg(ms)}^{-1}$, $[\text{V}]=\text{JC}^{-1}$.};
	\node at ($(MetalAct.north)+(0,2)$) [draw] [text width = 10cm] {
			$\varepsilon=E^{\circ}_{\text{cell}}=E^{\circ}_{\text{red}}(\text{cathode})-E^{\circ}_{\text{red}}(\text{anode})$, and $E^{\circ}_{\text{cell}\substack{\text{voltaic}\\ \text{galvanic}}}>0$. \\
			$\triangle G=-nFE$, where $F=96485\text{ C(mol)}^{-1}=96485\text{J (Vmol)}^{-1}$.\\
			$-nFE=-nFE^{\circ}+RT\ln Q$, so $E=E^{\circ}-\frac{RT}{nF}\ln Q$ (Nernst).\\ 
			$\triangle G=w_{\text{max}}=-nFE$. And $w=nFE_{\text{applied}}$. Also, $Q=It$.
	};
\end{tikzpicture}
