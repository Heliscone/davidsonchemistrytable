...
\begin{tikzpicture}[font=\sffamily]
  ...
  % Legend
	...
	\node at ($0.5*(H.west) + 0.5*(He.east)+(0,6.2)$) (Titles) [scale=2, font={\sffamily\Huge\bfseries}] {Periodic Table of Elements v3.1};
...
%%FORMULAS%%
	%Periodic Trends
	\node at ($(Ra.south)-(-1,4.85)$) (TrendOne) [draw, align=right]
	{EN, IE, EA, \& $\mathbb{Z}_{eff}$ increase $\rightarrow\uparrow$\\
	Radius \& Metallic decrease $\rightarrow\uparrow$};
	%GASSES AND ACIDS
	\node at ($(Fr.south)-(0.25,2.55)$) (Gas) [text width = 2cm, align=left] 
	{\textit{Gas}\\ NO\\ NO\textsubscript{2}\\ CO\\ CO\textsubscript{2}\\ CH\textsubscript{4}\\ C\textsubscript{2}H\textsubscript{6}\\ C\textsubscript{3}H\textsubscript{8}\\ C\textsubscript{4}H\textsubscript{10} \\ N\textsubscript{2}O NH\textsubscript{3}\\ SO\textsubscript{3} SO\textsubscript{2}\\ H\textsubscript{2}S HCl};
	\node at ($(Fr.south)+(0.1,-1.7)$) (SuperSeven) [text width=1cm,align=right] 
	{\textit{Super\textsuperscript{7}}\\ 
	HI\\ 
	HBr\\ 
	HCl\\ 
	HNO\textsubscript{3}\\ 
	...
	\node at ($(Units.south)-(-1.17,2.5)$) (Constants) {
	\begin{tabular}{rl}
	...
	Molar Gas Constant & $R=8.3144\text{ J mol}^{-1}\text{ K}^{-1}$\\
	$R=62.36\text{ L atm (mol K)}^{-1}$&$=0.08205746\text{L atm}\text{ K}^{-1}\text{ mol}^{-1}$\\
	Coulomb's Constant&$k_e=8.987551\times10^9\text{N m}^2\text{ C}^{-2}$\\
	Speed of Light (Vacuum) & $c=2.998\times 10^8\text{ m s}^{-1}$\\
	Boltzmann Consant & $k_b=1.3807\times10^{-23}\text{ J K}^{-1}$\\
	...
	\node at ($(Ti.north)+(0.5,4.5)$) (Equilibrium) [align=left] {
	\textbf{Equilibrium}\\
	$K_c=\frac{[\text{C}]^c[\text{D}]^d}{[\text{A}]^a[\text{B}]^b},$ where $a\text{A}+b\text{B}\rightleftharpoons c\text{C}+d\text{D}$. \\
$K_p=\frac{(P_C)^c(P_D)^d}{(P_A)^a(P_B)^b}$\\
$K_a=\frac{[\text{H}^+[\text{A}^-]}{[\text{HA}]}$\\
$K_b=\frac{[\text{OH}^-][\text{HB}^{+}]}{[\text{B}]}$\\
$K_w=K_aK_b=[\text{H}^+][\text{OH}^-]$\\
$=1.0\times10^{-14}$ ($25^{\circ}$ C)\\
$\text{pH}=-\log[\text{H}^+],\text{pOH}=-\log[\text{OH}^-]$\\
$\text{pH}+\text{pOH}=14$.\\
$\text{pH}=\text{p}K_a+\log\frac{[\text{A}^-]}{[\text{HA}]}$\\
$\text{p}K_a=-\log K_a$,$\text{p}K_b=-\log K_b$.
	};
	\node at ($(Constants.east)+(3,-1)$) (Kinetics) [align=left] {
	\textbf{Kinetics}\\
	$[A]_t-[A]_0=-kt$ (first order)\\
	$\ln[A]_t-\ln[A]_0=-k$ (second order)\\
	$\frac{1}{[A]_t}-\frac{1}{[A]_0}=kt$ (third order)\\
	$t_{1/2}=\frac{0.963}{t}$ (first order)
	};
	\node at ($(Equilibrium.east)+(2.2,-0.2)$) (Gasses) [align=left] {
	\textbf{Gasses/Solutions}\\
	$PV=nRT$\\
	...
	\node at ($(Gasses.east)+(2,0)$)	(Bzzt) [align=left] {
	\textbf{Thermo/Electrochem}\\
	$q=mc\triangle T$\\
	...
	\node at ($(Bzzt.north)+(-4.5,1.2)$) (Conversions) [align=left] {
	\textbf{Conversions}\\
	$1\text{ L}\cdot\text{atm}=101.3\text{ J}$\\
	$\text{K}= \phantom{}^{\circ}\text{C}+273.15$\\
	$1\text{ cal}=4.184\text{ J}$
	};
	\node at ($(Bzzt.north)+(0,2)$) (Quantum) [align=left] {
	\textbf{Quantum}\\
